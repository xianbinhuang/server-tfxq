...
type (
	outer struct {
		...
		Name        string `xorm:"char(50)" json:"name"`
		Telephone string `xorm:"char(50)" json:"telephone"`
		Company string `xorm:"char(50)" json:"company"`
	//	time_s string `xorm:"char(50)" json:"time_s"`
	//	time_e string `xorm:"char(50)" json:"time_e"`
	//	work string `xorm:"char(50)" json:"work"`
	}
	outerPage struct {
		List       []outer `json:"list"`
		Size       int `json:"size"`
...
func init() {
	...
	使用curl测试接口
	POST   /outer     创建
	curl -X POST -d '{"title":"x"}' http://localhost:4000/outer

	GET    /outer/1 查看
	curl http://localhost:4000/outer/1

	GET    /outers
	curl http://localhost:4000/outers

	DELETE /outer/1 删除
	curl -X DELETE http://localhost:4000/outer/1

	PUT    /outer/1 更新
	curl -X PUT -d '{"title":"xxx"}' http://localhost:4000/outer/1
	*/
	...
	sub.Get(fmt.Sprintf("/%ss", MODEL_NAME), func(c *iris.Context) {
		...
		var data = outerPage{Page:page,Size:size}
		...
		if err := web.DB.Where("user_id = ?", my.ID).Desc("id").Limit(data.Size, offset).Find(&data.List); err == nil {
			if total, err := web.DB.Where("user_id = ?", my.ID).Count(new(outer)); err == nil {
